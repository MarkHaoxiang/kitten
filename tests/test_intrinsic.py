...
class TestIntrinsicCuriosityModule:
    def build_networks(self, in_features=5, encoding_size=3, action_size=2):
        ...
        inverse_head = nn.Linear(
            in_features=encoding_size * 2, out_features=action_size
        )
    ...
    def test_icm(
        ...
        feature_net, forward_head, inverse_head = self.build_networks(
            in_features=obs_size, encoding_size=encoding_size, action_size=action_size
        )
        ...
        ), f"Expected encoding shape {(*batch_size, encoding_size)} but got {phi_0.shape}"
        assert (
            pred_phi_1.shape == phi_1.shape
        ), f"Shape of predicted encoding {pred_phi_1.shape} does not match true encoding {phi_1.shape}"
        assert (
            pred_a.shape == a.shape
        ), f"Shape of predicted action {pred_a.shape} does not match true action {a.shape}"

        # Reward
        r_i = icm(s_0, s_1, a)
        assert (
            r_i.shape == batch_size
        ), f"Reward shape {r_i.shape} should match batch size {batch_size}"
