...
class TestReplayBuffer:
    ...
    def test_replay_insert(self, capacity: int, number: int, shape: Sequence[int]):
        replay_buffer = ReplayBuffer(capacity=capacity, shape=shape, device=DEVICE)
        ...
        assert len(replay_buffer) == min(
            capacity, number * 2
        ), f"Incorrect number of elements contained. Expected {min(capacity, number*2)} but got {len(replay_buffer)}"

    @pytest.mark.parametrize("number", [100, 1000])
    def test_replay_sample(self, number: int, capacity: int = 10000):
        replay_buffer = ReplayBuffer(capacity=capacity, shape=(1,), device=DEVICE)
        replay_buffer.append((torch.arange(capacity).unsqueeze(-1),))

        # Random sample
        sample, aux = replay_buffer.sample(number)
        for x in sample:
...
class TestPrioritizedReplayBuffer:
    ...
    @pytest.mark.parametrize("number", [100, 1000])
    def test_replay_sample(self, number: int, capacity: int = 1000):
        replay_buffer = PrioritizedReplayBuffer(
        ...
        replay_buffer.append((torch.arange(capacity).unsqueeze(-1),))
        # Random sample
        sample = replay_buffer.sample(number)[0]
        for x in sample:
            assert x[0] in replay_buffer.storage[0], "Samples not in replay buffer"
    ...
    def test_manual_case(self):
        ...
        assert torch.all(
            torch.eq(
                replay_buffer.storage[0].squeeze(),
                torch.tensor([1, 2, 3, 0, 0, 0], device=DEVICE),
            )
        )
        assert np.all(
            replay_buffer.sum_tree == np.array([3, 3, 0, 2, 1, 0, 0, 1, 1, 1, 0, 0, 0])
        )
        replay_buffer.sample(3)
        assert np.all(
            replay_buffer.sum_tree == np.array([9, 9, 0, 5, 4, 0, 0, 2, 3, 4, 0, 0, 0])
        )
